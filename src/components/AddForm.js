...
// Actions
import { addSmurf, addSmurfError } from "../actions";
...
const AddForm = (props) => {
  ...
  const { smurfErrorMessage, addSmurf, addSmurfError } = props;
  ...
  const handleSubmit = (e) => {
    ...
    if (state.name === "" || state.position === "" || state.nickname === "") {
      //add in error action
      addSmurfError(
        "To create a new smurf, you must input a name, position, and nickname."
      );
    } else {
      addSmurf(state);
...
const mapStateToProps = (state) => {
  return { smurfErrorMessage: state.smurfErrorMessage };
};

export default connect(mapStateToProps, { addSmurf, addSmurfError })(AddForm);
